...
if __name__ == "__main__":
    ...
    # Test user
    user_id = add_user("testuser", "test@example.com", "hashedpassword123")
    print("Added user with ID:", user_id)

    # Add a conversation
    add_conversation(user_id, "Hello", "Hi, how can I help you?")
    convs = get_conversations(user_id)
    print("Conversations:", convs)

    # Add RAG context
    sample_embedding = [0.1, 0.2, 0.3]
    add_rag_context(user_id, "Eat more vegetables", sample_embedding)
    rag = get_rag_context(user_id)
    print("RAG Context:", rag)
